...
Recently I resumed PPE allocation duties for a couple of weeks and took the opportunity to automate a checklist of the orders sorted by region to help with their packaging for delivery. As a data person, I couldn't help but also sort the names of agencies after region. The resulting data surprised me when I noticed that the  secondary sort by agency names placed agencies with names in ALL CAPS came before other agencies with Title Case names. 
...
***Verdict:***: Title Case city names precede ALL CAPS city names per AVALON CARE CENTER - FEDERAL WAY, LLC in FEDERAL WAY. Also true for REDMOND CARE AND REHABILITATION CENTER in REDMOND.
...
***Verdict:***: ALL CAPS city names precede Title Case city names per AVALON CARE CENTER - FEDERAL WAY, LLC in FEDERAL WAY. Things seem even weirder for REDMOND and RENTON where the 2 agencies with ALL CAPS cities precede the other 2 agencies in Redmond and Renton. 
...
Rather than tackle agency names which likely have legitimate UPPER CASE initials and acronyms, formatting the city names will at least reveal where `dplyr::arrange` puts AVALON CARE CENTER - FEDERAL WAY, LLC once FEDERAL WAY is formatted. To  [stringr::str_to_title](https://stringr.tidyverse.org/reference/case.html)! 
...
***Verdict:***: With my city data now formatted as Title Case, the secondary sorting of names is returned as hoped for both Federal Way, Redmond and Seattle agencies. ***WINNER WINNER (TOFU DINNER!)***
...
***Verdict:***: BETTER, but not all there yet. Secondary sort on mixed case names results in agency names starting with a given letter in ALL CAPS preceding other Title Cased names of same letter. ***RUNNER UP***.

## A Sorted Affair

As often is the case, thinking that a simple ordering of data is at most a 5 minute task, inconsistent data may quickly beckon one down a rabbit hole. An interesting, but also frustrating one. 

***Tidyverse: 1 Data.Table: 0***

## Conclusions

1. Yes, there is inconsistency between `dplyr::arrange` and `data.table::setorder` when sorting by mixed case characters.
2. Unless you know the software it may be not be obvious whether ALL CAPS has better chance of showing up first in the data, but much to my surprise, it seems like it's a solid gamble to upper cases likely sorted fields.